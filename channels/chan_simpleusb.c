...
/* needed if we include the sound routines */
#ifndef	NEW_ASTERISK
...
/* sound routines, not sure if they are needed in the newer versions of asterisk */
#ifndef	NEW_ASTERISK
	...
	if (!strncmp(text,"PAGE",4))
	{
		...
		for(i = 0; i < audio_samples; i += FRAME_SIZE)
		{
			...
			wf.frametype = AST_FRAME_VOICE;
		        wf.subclass.integer = AST_FORMAT_SLINEAR;
...
{
	/* needed if we inlucde the sound routines */
#ifndef	NEW_ASTERISK
...
static int simpleusb_answer(struct ast_channel *c)
{
	/* this and the snippet below are needed if we include the sound routines */
#ifndef	NEW_ASTERISK
	struct chan_simpleusb_pvt *o = c->tech_pvt;
#endif
...
static int susb_tune(int fd, int argc, char *argv[])
{
	struct chan_simpleusb_pvt *o = find_desc(simpleusb_active);
	int i=0;
...
*/
static int susb_set_debug(int fd, int argc, char *argv[])
{
	struct chan_simpleusb_pvt *o = find_desc(simpleusb_active);
...
static int susb_set_debug_off(int fd, int argc, char *argv[])
{
	struct chan_simpleusb_pvt *o = find_desc(simpleusb_active);
...
static int susb_active(int fd, int argc, char *argv[])
{
        if (argc == 2)
