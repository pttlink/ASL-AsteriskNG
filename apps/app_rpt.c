 ...
 * Ported to Asterisk 1.8 by Stacy Olivas, KG7QIN <kg7qin@arrl.net>                               
 *
...
#ifdef DEVEL_DEBUG
	#define DEBUG_LOGGER(...)  ast_log(__VA_ARGS__)
	#define DEBUG_QUICK(x)     ast_log(LOG_NOTICE, "%s\n",x)
	#define DEBUG_STATENTER(x) ast_log(LOG_NOTICE, "Entering: %s\n", x)
	#define DEBUG_STATEXIT(x)  ast_log(LOG_NOTICE, "Leaving : %s\n", x)
	
	/* macro to expand variables passed, showing name and value */
	
	#define DEBUG_VAR_DATA(...) ast_log(LOG_NOTICE, "Variable data: "#__VA_ARGS__" = %s\n", __VA_ARGS__)
	
#else
	#define DEBUG_LOGGER(...)
	#define DEBUG_QUICK(x)
	#define DEBUG_STATENTER(x)
	#define DEBUG_STATEXIT(x)
	#define DEBUG_VAR_DATA(...)
#endif
...
static  char *tdesc = "Radio Repeater / Remote Base  version 0.399 07/26/2018";
...
pthread_t id;
	...
	DEBUG_STATENTER("_rpt_mutex_lock");
	id = pthread_self();
	ast_mutex_lock(&locklock);
	t = put_lockthread(id);
	if (!t)
...
pthread_t id;
	...
	DEBUG_STATENTER("_rpt_mutex_unlock");
	id = pthread_self();
	ast_mutex_lock(&locklock);
	t = put_lockthread(id);
	if (!t)
...
int	ms, res = 0;
	...
	DEBUG_LOGGER(LOG_NOTICE, "* Requesting psuedo channel: dialstr = %s\n",dialstr);
	dest = ast_request("IAX2", AST_FORMAT_SLINEAR, chan, dialstr ,NULL);
	if (!dest)
	{
		DEBUG_QUICK("- - !dest");
		if (ast_safe_sleep(chan,150) == -1) {
			DEBUG_QUICK("ast_safe_sleep(chan,150) == -1");
			return;
		}
		DEBUG_LOGGER(LOG_NOTICE, "** Requesting another pseudo channel.  dialstr = %s\n", dialstr);
		dest = ast_request("IAX2", AST_FORMAT_SLINEAR, chan, dialstr ,NULL);
		if (!dest)
		{
			DEBUG_QUICK("- - !dest");
	...
	DEBUG_LOGGER(LOG_NOTICE, "rpt forwarding call from %s to %s on %s\n", nodefrom,dialstr,dest->name);
	if (option_verbose > 2)
		ast_verbose(VERBOSE_PREFIX_3 "rpt forwarding call from %s to %s on %s\n",
			nodefrom,dialstr,dest->name);
	ast_set_callerid(dest,nodefrom,chan->caller.id.name.str,nodefrom);
        res = ast_call(dest,dialstr,999);
        DEBUG_LOGGER(LOG_NOTICE," ast_call returned %d\n", res);
	cs[0] = chan;
	cs[1] = dest;
	for(;;)
	{
		if (ast_check_hangup(chan)) {
			DEBUG_QUICK("- ast_check_hangup(chan)");
			break;
		}
		if (ast_check_hangup(dest)) {
			DEBUG_QUICK("- ast_check_hangup(dest)");
			break;
		}
		ms = 100;
		w = cs[0];
		...
		if (w == chan)
		{
			...
			{
				DEBUG_QUICK("- ast_frfre(f)");
				ast_frfree(f);
				break;
			}
			DEBUG_QUICK("ast_write(chan,f)");
			ast_write(dest,f);
			ast_frfree(f);
		}
		if (w == dest)
...
static inline void goertzel_sample(goertzel_state_t *s, short sample)
{
	...
	DEBUG_STATENTER("goertzel_sample");
	v1 = s->v2;
	s->v2 = s->v3;
...
static inline void goertzel_update(goertzel_state_t *s, short *samps, int count)
{
	...
	DEBUG_STATENTER("goertzel_update");
	for (i = 0; i < count; i++) {
...
{
	DEBUG_STATENTER("goertzel_result");
	goertzel_result_t r;
...
{
	DEBUG_STATENTER("goertzel_reset");
	s->v2 = s->v3 = s->chunky = 0.0;
}
...
static void tone_detect_init(tone_detect_state_t *s, int freq, int duration, int amp)
{
	...
	DEBUG_STATENTER("tone_detech_init");
	s->freq = freq;
...
int	myms = timeoutms;
	...
	DEBUG_STATENTER("serial_rxready");
...
static int serial_rxflush(int fd, int timeoutms)
{
	...
	DEBUG_STATENTER("serial_rxflush");
...
static int serial_rx(int fd, char *rxbuf, int rxmaxbytes, unsigned timeoutms, char termchr)
{
	...
	if(i && debug >= 6) {
		...
		ast_log(LOG_NOTICE, "String returned was:\n");
		for(j = 0; j < i; j++)
			ast_log(LOG_NOTICE, "%02X ", (unsigned char ) rxbuf[j]);
		ast_log(LOG_NOTICE, "\n");
	}
	DEBUG_STATEXIT("serial_rx");
	return i;
}
...
static int serial_txstring(int fd, char *txstring)
{
	...
	DEBUG_STATENTER("serial_txstring");
	...
	DEBUG_STATEXIT("serial_txstring");
	return 0;
}
...
static int handle_meter_tele(struct rpt *myrpt, struct ast_channel *mychannel, char *args)
{
	...
	DEBUG_STATENTER("handle_meter_telem");	
...
static int function_meter(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	DEBUG_STATENTER("function_meter");
	if (myrpt->remote) {
		DEBUG_STATEXIT("function_meter");
		return DC_ERROR;
	}
	...
	DEBUG_QUICK("- function_meter:  rpt_telem_select(myrpt,command_source,mylink);");
	rpt_telem_select(myrpt,command_source,mylink);
	DEBUG_QUICK(" - function_meter: rpt_telemetry(myrpt,METER,param);");
...
static int function_userout(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	DEBUG_STATENTER("function_userout");
	if (myrpt->remote) {
		DEBUG_STATEXIT("function_userout");
		return DC_ERROR;
	}

		ast_log(LOG_NOTICE, "userout param = %s, digitbuf = %s\n", (param)? param : "(null)", digitbuf);
	
	DEBUG_QUICK("- function_userout: rpt_telem_select(myrpt,command_source,mylink);");
	rpt_telem_select(myrpt,command_source,mylink);
	DEBUG_QUICK(" - function_userout: rpt_telemetry(myrpt,USEROUT,param);");
...
static int function_cmd(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	DEBUG_STATENTER("function_cmd");
	if (myrpt->remote) {
		DEBUG_STATEXIT("function_cmd");
		return DC_ERROR;
	}
...
static void flush_telem(struct rpt *myrpt)
{
	...
	while(telem != &myrpt->tele)
	{
		if (telem->mode != SETREMOTE) {
			DEBUG_QUICK("* performing soft hangup");
			ast_softhangup(telem->chan,AST_SOFTHANGUP_DEV);
		}
		telem = telem->next;
	}
	DEBUG_STATEXIT("flush_telem");
	rpt_mutex_unlock(&myrpt->lock);
}
/*
...
int	src;
	...
	DEBUG_STATENTER("rpt_telem_select");
	if (mylink && mylink->chan)
	{
		src = LINKMODE_GUI;
		if (mylink->phonemode) {
			DEBUG_QUICK("LINKMODE_PHONE");
			src = LINKMODE_PHONE;
		}
		else if (!strncasecmp(mylink->chan->name,"echolink",8)) {
			DEBUG_QUICK("LINKMODE_ECHOLINK");
			src = LINKMODE_ECHOLINK;
		}
		else if (!strncasecmp(mylink->chan->name,"tlb",8)) {
			DEBUG_QUICK("LINKMODE_TLB");
			src = LINKMODE_TLB;
		}
		if (myrpt->p.linkmodedynamic[src] && (mylink->linkmode >= 1) && 
		    (mylink->linkmode < 0x7ffffffe)) {
				DEBUG_QUICK("mylink->linkmode = LINK_HANG_TIME");
				mylink->linkmode = LINK_HANG_TIME;
		}
	}
	if (!myrpt->p.telemdynamic) {
		DEBUG_QUICK("!myrpt->p.telemdynamic");
		DEBUG_STATEXIT("rpt_telem_select");
		return;
	}
	if (myrpt->telemmode == 0) {
		DEBUG_QUICK("myrpt->telemmode == 0");
		DEBUG_STATEXIT("rpt_telem_select");
		return;
	}
	if (myrpt->telemmode == 0x7fffffff) {
		DEBUG_QUICK("myrpt->telemmode == 0x7fffffff");
		DEBUG_STATEXIT("rpt_telem_select");
		return;
	}
	DEBUG_QUICK("myrpt->telemmode = TELEM_HANG_TIME");
...
char	*val;
	...
	DEBUG_STATENTER("local_dtmf_helper");
	...
	DEBUG_STATEXIT("local_dtmf_helper");
	return;
}
...
int	i,spos;
	...
	DEBUG_STATENTER("__nklinklist");
	buf[0] = 0; /* clear output buffer */
	if (myrpt->remote) {
		DEBUG_STATEXIT("__mklinklist");
		return;
	}
	/* go thru all links */
...
struct rpt_link *l;
	...
	DEBUG_STATENTER("__kickshort");
	for(l = myrpt->links.next; l != &myrpt->links; l = l->next)
	{
		/* if is not a real link, ignore it */
		DEBUG_LOGGER(LOG_NOTICE, "l->name[0] = %c\n", l->name[0]);
		if (l->name[0] == '0') {
			DEBUG_QUICK("not a real link, ignoring");
			continue;
		}
...
struct ast_var_t *newvariable;
	...
	DEBUG_STATENTER("rpt_event_process");
	if (!starttime) {
		DEBUG_QUICK("-- !starttime");
		DEBUG_STATEXIT("rpt_event_process");
		return;
	}
	for (v = ast_variable_browse(myrpt->cfg, myrpt->p.events); v; v = v->next)
	{
		...
		if (argc != 3)
		{
			DEBUG_LOGGER(LOG_NOTICE, "argc != 3, event exec item malformed: %s\n", v->value);
		...
		{
			DEBUG_LOGGER(LOG_NOTICE, "Unrecognized event action (%c) in exec item malformed: %s\n",action,v->value);
...
//				pbx_builtin_setvar_helper(myrpt->rxchannel,cmpvar,var);
		...
		if (action == 'C') /* excecute a command */
		{
			...
			DEBUG_VAR_DATA(maxActions);
			for(i = 0 ; i < maxActions; i++)
			{
				if(!strncasecmp(argv[0], function_table[i].action, l))
				{
					DEBUG_LOGGER(LOG_NOTICE, "thisAction = %d", i);
					thisAction = i;
					break;
				} 
			} 
			...
			DEBUG_LOGGER(LOG_NOTICE, "Event on node %s doing rpt command %s for condition %s\n",
					myrpt->name,cmd,v->value);
			if (option_verbose > 2)
			...
			if (myrpt->cmdAction.state == CMD_STATE_IDLE)
			{
				...
				DEBUG_VAR_DATA(myrpt->cmdAction.param);
				DEBUG_VAR_DATA(myrpt->cmdAction.digits);
			} 
			else
			{
		...
		{
			DEBUG_QUICK("-> execute a shell command");
			char *cp;

			DEBUG_LOGGER(LOG_NOTICE, "Event on node %s doing shell command %s for condition %s\n",
					myrpt->name,cmd,v->value);
			if (option_verbose > 2)
...
int	n;
	...
	DEBUG_STATENTER("rpt_update_links");
	...
	DEBUG_STATEXIT("rpt_update_links");
	return;
}
...
static struct ast_config *ourcfg;
	...
	DEBUG_STATENTER("forward_node_lookup");
	...
	for(i = 0; i < n; i++)
	{
		DEBUG_LOGGER(LOG_NOTICE, "-> i = %d\n", i);
		/* if file does not exist */
		if (stat(strs[i],&mystat) == -1) { 
			DEBUG_QUICK("stat(strs[i],&mystat) == -1");
			continue;
		}
		...
		/* if file not there, try next */
		if (!ourcfg) {
			DEBUG_QUICK("-- file not there, try next");
			continue;
		}
		if (!val) {
			DEBUG_QUICK("-- !val (for loop)");
	...
	ast_free(efil);
	DEBUG_VAR_DATA(val);
	DEBUG_LOGGER(LOG_NOTICE, "-> val = %s", val);
	DEBUG_STATEXIT("forward_node_lookup");
	return(val);
}
	...
	for(i = 0; i < nrpts; i++)
	{
		if (!strcmp(argv[2],rpt_vars[i].name)){
			...
			for(s = s_head.next; s != &s_head; s = s->next){
				...
				char conntime[21];
				hours = (int) connecttime/3600000L;
				connecttime %= 3600000L;
				minutes = (int) connecttime/60000L;
				connecttime %= 60000L;
...
static int rpt_do_playback(int fd, int argc, char *argv[])
{
	...
	DEBUG_STATENTER("rpt_do_playback");
        if (argc != 4) return RESULT_SHOWUSAGE;
...
static int rpt_do_localplay(int fd, int argc, char *argv[])
{
        ...
        DEBUG_STATENTER("rpt_do_localplay");
        if (argc != 4) return RESULT_SHOWUSAGE;
...
static int rpt_do_sendtext(int fd, int argc, char *argv[])
{
	...
	DEBUG_STATENTER("rpt_do_sendtext");
...
static int rpt_push_alt_macro(struct rpt *myrpt, char *sptr)
{
	...
	DEBUG_STATENTER("rpt_push_alt_macro");
	rpt_mutex_lock(&myrpt->lock);
...
static int play_tone_pair(struct ast_channel *chan, int f1, int f2, int duration, int amplitude)
{
	...
	DEBUG_STATENTER("play_tone_pair");
...
static int send_tone_telemetry(struct ast_channel *chan, char *tonestring)
{
	...
	DEBUG_STATENTER("send_tone_telemetry");
	if(!tonestring)
		return res;
...
int	res;
	...
	DEBUG_STATENTER("sayphoneticstr");
        ...
        switch(type){
                case DLY_TELEM:
                        if(wait_times)
                                ...
                                interval = 1000;
                        DEBUG_LOGGER(LOG_NOTICE, "<-> DLY_TELEM / interval = %d\n", interval);
                        break;
                ...
                case DLY_ID:
                        if(wait_times)
                                ...
                                interval = 500;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_ID / interval = %d\n", interval);
                        break;
                ...
                case DLY_UNKEY:
                        if(wait_times)
                                ...
                                interval = 1000;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_UNKEY / interval = %d\n", interval);
                        break;
                ...
                case DLY_LINKUNKEY:
                        if(wait_times)
                                ...
                                interval = 1000;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_LINKUNKEY / interval = %d\n", interval);
                        break;
                ...
                case DLY_CALLTERM:
                        if(wait_times)
                                ...
                                interval = 1500;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_CALLTERM / interval = %d\n", interval);
                        break;
                ...
                case DLY_COMP:
                        if(wait_times)
                                ...
                                interval = 200;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_COMP / interval = %d\n", interval);
                        break;
                ...
                case DLY_PARROT:
                        if(wait_times)
                                ...
                                interval = 200;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_PARROT / interval = %d\n", interval);
                        break;
                case DLY_MDC1200:
                        if(wait_times)
                                ...
                                interval = 350;
                        DEBUG_LOGGER(LOG_NOTICE, "- DLY_MDS1200 / interval = %d\n", interval);
                        break;
                default:
			interval = 0;
			DEBUG_LOGGER(LOG_NOTICE, "- default / interval = %d\n", interval);
			break;
        }

	if(wait_times_save) {
		DEBUG_QUICK("--- ast_free(wait_times_save)");
       		ast_free(wait_times_save);
       	}

       	DEBUG_LOGGER(LOG_NOTICE, "Exiting get_wait_interval / interval = %d\n", interval);
	return interval;
}                                                                                                                  
...
static int wait_interval(struct rpt *myrpt, int type, struct ast_channel *chan)
{
	...
	DEBUG_STATENTER("wait_interval");
	do {
		DEBUG_QUICK("- do loop start");
		while (myrpt->p.holdofftelem && 
			(myrpt->keyed || (myrpt->remrx && (type != DLY_ID))))
		{
			DEBUG_QUICK("myrpt->p.holdofftelem && (myrpt->keyed || (myrpt->remrx && (type != DLY_ID)))");
			if (ast_safe_sleep(chan,100) < 0) {
				DEBUG_QUICK("ast_safe_sleep(chan,100) < 0");
				return -1;
			}
		}

		DEBUG_QUICK(" -- get_wait_interval(myrpt, type");
		interval = get_wait_interval(myrpt, type);
		DEBUG_LOGGER(LOG_NOTICE, "Delay interval = %d\n", interval);
		if(debug)
			ast_log(LOG_NOTICE,"Delay interval = %d\n", interval);
		if(interval) {
			DEBUG_LOGGER(LOG_NOTICE, "Executing: ast_safe_sleep(chan,interval) < 0\n");
			if (ast_safe_sleep(chan,interval) < 0) return -1;
		}
		DEBUG_LOGGER(LOG_NOTICE, "Delay complete\n");
		if(debug)
			ast_log(LOG_NOTICE,"Delay complete\n");
	}
	while (myrpt->p.holdofftelem && 
		(myrpt->keyed || (myrpt->remrx && (type != DLY_ID))));
	DEBUG_STATEXIT("wait_interval");
	return 0;
}
...
struct	tm localtm;
	...
	DEBUG_STATENTER("handle_varcmd_tele");
	...
	{
		DEBUG_QUICK("- CONNECTED");
		if (n < 3) {
			DEBUG_QUICK("- n < 3");
			return;
		}
		res = wait_interval(myrpt, DLY_TELEM,  mychannel);
		DEBUG_LOGGER(LOG_NOTICE, "wait_interval(myrpt, DLY_TELEM,  mychannel) == %d\n", res);
		if (res == -1) {
			DEBUG_QUICK("wait_interval(myrpt, DLY_TELEM,  mychannel) == -1");
			return;
		}
		...
		if (!res) {
			...
			res = ast_waitstream(mychannel, "");
			 ...
			 ast_log(LOG_WARNING, "ast_streamfile failed on %s\n", mychannel->name);
		DEBUG_QUICK("-- ast_stopstream(mychannel)");
		ast_stopstream(mychannel);
		...
		DEBUG_STATEXIT("handle_varcmd_tele");
		return;
	}
	if (!strcasecmp(strs[0],"CONNFAIL"))
	{
		DEBUG_QUICK("- CONNFAIL");
		if (n < 2) return;			
		if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) return;
...
struct	mdcparams *mdcp;
	...
	DEBUG_STATENTER("rpt_tele_thread");
	/* get a pointer to myrpt */
	DEBUG_QUICK("myrpt = mytele->rpt;");
	myrpt = mytele->rpt;

	/* Snag copies of a few key myrpt variables */
	DEBUG_QUICK("- snag a copies of a few key myrpt vars");
	rpt_mutex_lock(&myrpt->lock);
	...
	if(!nodename)
	{
	    ...
	    rpt_mutex_unlock(&myrpt->lock);
	    ast_free(mytele);
	    DEBUG_STATEXIT("rpt_tele_thread");
	    pthread_exit(NULL);
	}

	if (myrpt->p.ident){
		DEBUG_LOGGER(LOG_NOTICE, "myrpt->p.ident = %s\n", myrpt->p.ident);
	...
	DEBUG_QUICK("***** Allocating a pseudo channel");
...
#endif
	DEBUG_QUICK("- answering mychannel");
	...
	DEBUG_QUICK("- attempting to make this a conference");
	ci.chan = 0;
	...
	DEBUG_QUICK("ci.confno = (((mytele->mode == ID1) || (mytele->mode == PLAYBACK) || (mytele->mode == TEST_TONE) || (mytele->mode == STATS_GPS_LEGACY)) ? myrpt->conf : myrpt->txconf);");
	DEBUG_LOGGER(LOG_NOTICE, "----> mytele->mode = %d\n", mytele->mode);
	ci.confno = (((mytele->mode == ID1) || (mytele->mode == PLAYBACK) ||
	    (mytele->mode == TEST_TONE) || (mytele->mode == STATS_GPS_LEGACY)) ? 
		myrpt->conf : myrpt->txconf);
	DEBUG_QUICK("ci.confmode = DAHDI_CONF_CONFANN;");
	ci.confmode = DAHDI_CONF_CONFANN;
	/* first put the channel on the conference in announce mode */
	DEBUG_QUICK("- first put the channel on the conference in announce mode");
	...
	{
	    case USEROUT:
	    	DEBUG_LOGGER(LOG_NOTICE, "- USEROUT; %s \n", mytele->param);
		handle_userout_tele(myrpt, mychannel, mytele->param);
		imdone = 1;
		break;

	    case METER:
	    	DEBUG_LOGGER(LOG_NOTICE, "- METER; %s \n", mytele->param);
		handle_meter_tele(myrpt, mychannel, mytele->param);
		imdone = 1;
		break;

	    case VARCMD:
	    	DEBUG_LOGGER(LOG_NOTICE, "- VARCMD; %s \n", mytele->param);
...
treataslocal:
		...
		{
			DEBUG_QUICK("(hasremote && ((!myrpt->cmdnode[0]) || (!strcmp(myrpt->cmdnode,'aprstt')))");
			/* set for all to hear */
			ci.chan = 0;
		...
		if (myrpt->p.locallinknodesn)
		{
			...
			if (w) 
			{
				...
				*/
				...
				while(myrpt->unkeytocttimer)
				{
					DEBUG_QUICK("wait for telemetry to expire");
					int ctint;
					if(myrpt->unkeytocttimer > 100)
						ctint = 100;
					else
			...
			DEBUG_QUICK("goto treataslocal");
			goto treataslocal;
		}
		if (myrpt->p.nolocallinkct) /* if no CT if this guy is on local system */
		{
			DEBUG_QUICK("if no CT if this guy is on local system");
			int v,w;
			w = 0;
			for(v = 0; v < nrpts; v++)
			{
		...
		*/
		...
		{
			DEBUG_QUICK("wait for telemetry timer to expire");
			int ctint;
	    ...
	    case PLAYBACK:
            case LOCALPLAY:
            	DEBUG_QUICK("- PLAYBACK/LOCALPLAY");
		/* wait a little bit */
		if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) break;
	    ...
	    case REMSHORTSTATUS:
	    case REMLONGSTATUS:
	    	DEBUG_QUICK("- REMSHORTSTATUS/REMLONGSTATUS");
		if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) break;
		res = saynode(myrpt,mychannel,myrpt->name);
		if(!res)
	    ...
	    case ACT_TIMEOUT_WARNING:
	    	DEBUG_QUICK("- ACT_TIMEOUT_WARNING");
		time(&t);
		res = saynode(myrpt,mychannel,myrpt->name);
		if (!res)
	    ...
	    case STATS_TIME:
            case STATS_TIME_LOCAL:
            	DEBUG_QUICK("- STATS_TIME/STATS_TIME_LOCAL");
	    	if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) break;
		t = time(NULL);
		rpt_localtime(&t, &localtm, myrpt->p.timezone);
		t1 = rpt_mktime(&localtm,NULL);
	    ...
	    case STATS_GPS_LEGACY:
	    	DEBUG_QUICK("- STATS_GPS/STATS_GPS_LEGACY");
		fp = fopen(GPSFILE,"r");
		if (!fp) break;
		...
		break;
	    default:
	    	DEBUG_QUICK("- Fall through to default");
	    	break;
	}
	DEBUG_QUICK("---- exited swtich\n");
	if (!imdone)
	{
		DEBUG_QUICK("- !imdone\n");
		if (!res) {
			DEBUG_QUICK("res = ast_waitstream(mychannel, '');");
			res = ast_waitstream(mychannel, "");
		}
		else {
			ast_log(LOG_WARNING, "ast_streamfile failed on %s\n", mychannel->name);
	...
	DEBUG_QUICK("ast_stopstream(mychannel);");
	ast_stopstream(mychannel);
	DEBUG_QUICK("pt_mutex_lock(&myrpt->lock);");
	rpt_mutex_lock(&myrpt->lock);
	if (mytele->mode == TAILMSG)
	{
		DEBUG_QUICK("-- mytel->mode == TAILMSG");
		if (!res)
		{
			DEBUG_QUICK("- !res");
			myrpt->tailmessagen++;
		...
		{
			DEBUG_QUICK("- res is true");
			myrpt->tmsgtimer = myrpt->p.tailsquashedtime;
		}
	}
	DEBUG_QUICK("Running:  remque((struct qelem *)mytele);");
	remque((struct qelem *)mytele);
	myrpt->active_telem = NULL;
	DEBUG_QUICK("rpt_mutex_unlock(&myrpt->lock);");
	rpt_mutex_unlock(&myrpt->lock);
	DEBUG_QUICK("ast_free(nodename);");
	ast_free(nodename);
	if(id_malloc) {
		DEBUG_QUICK("ast_free(ident);");
		ast_free(ident);
	}
	DEBUG_QUICK("ast_free(mytele);");
	ast_free(mytele);		
	DEBUG_QUICK("ast_hangup(mychannel);");
	ast_hangup(mychannel);
#ifdef  APP_RPT_LOCK_DEBUG
	{
		struct lockthread *t;
...
struct rpt_link *l;
	...
	{
		DEBUG_QUICK("- rpt_telemetry: (mode == ID) && is_paging(myrpt))");
		myrpt->deferid = 1;
		return;
	}

	DEBUG_LOGGER(LOG_WARNING, "- rpt_telemetry:  mode = %d\n", mode);
	switch(mode)
	{
	    case CONNECTED:
	    	DEBUG_QUICK("*- CONNECTED");
		...
		if ((mylink->name[0] == '3') && (!myrpt->p.eannmode)) { 
			DEBUG_QUICK("True:  ((mylink->name[0] == '3') && (!myrpt->p.eannmode))");
			return;
		}
		break;
	    case REMDISC:
	...
	{
		DEBUG_QUICK("!myrpt->remote -- not a remote");
		...
		switch(mode)
		{
			...
			DEBUG_LOGGER(LOG_NOTICE, "send_tele_link(myrpt,mystr) / mystr = %s\n", mystr);
...
struct ast_channel *mychannel,*genchannel,*c;
	...
	DEBUG_STATENTER("rpt_call");
	myrpt->mydtmf = 0;
	/* allocate a pseudo-channel thru asterisk */
	DEBUG_LOGGER(LOG_NOTICE, "* Allocating pseudo channel for rpt_call\n");
	mychannel = ast_request("DAHDI", AST_FORMAT_SLINEAR, NULL, "pseudo", NULL);
	if (!mychannel)
	{
		ast_log(LOG_ERROR,"rpt:Sorry unable to obtain pseudo channel\n");
		DEBUG_STATEXIT("rpt_call");
		pthread_exit(NULL);
	}
#ifdef	AST_CDR_FLAG_POST_DISABLED
	if (mychannel->cdr)
		ast_set_flag(mychannel->cdr,AST_CDR_FLAG_POST_DISABLED);
#endif
	DEBUG_LOGGER(LOG_NOTICE, "- rpt_call: Answering channel 'mychannel'\n");
	ast_answer(mychannel);
	ci.chan = 0;
	...
	DEBUG_QUICK("- rpt_call:  putting channel 'mychannel' on conference");
	if (ioctl(mychannel->fds[0],DAHDI_SETCONF,&ci) == -1)
	{
		ast_log(LOG_WARNING, "Unable to set conference mode to Announce\n");
		ast_hangup(mychannel);
	...
	/* allocate a pseudo-channel thru asterisk */
	DEBUG_QUICK("rpt_call:  allocating a psuedo channel");
...
#endif
	DEBUG_LOGGER(LOG_NOTICE, "- rpt_call: Answering channel 'genchannel'\n");
	ast_answer(genchannel);
	ci.chan = 0;
	ci.confno = myrpt->conf;
	ci.confmode = DAHDI_CONF_CONF | DAHDI_CONF_TALKER | DAHDI_CONF_LISTENER;
	/* first put the channel on the conference */
	DEBUG_QUICK("- rpt_call: putting channel 'genchannel' on conference");
	...
	{
		DEBUG_QUICK("- rpt_call: (myrpt->patchexten[0])");
		strcpy(myrpt->exten,myrpt->patchexten);
		myrpt->callmode = 2;
	}
	while ((myrpt->callmode == 1) || (myrpt->callmode == 4))
	{
		DEBUG_QUICK("- rpt_call: (myrpt->callmode == 1) || (myrpt->callmode == 4)");
	...
	DEBUG_QUICK("- rpt_call:  stopping any tone generation");
	tone_zone_play_tone(genchannel->fds[0],-1);
	/* end if done */
	if (!myrpt->callmode)
	{
		DEBUG_QUICK("- rpt_call: !myrpt->callmode");
		if(debug)
			ast_log(LOG_NOTICE, "callmode==0\n");
		DEBUG_QUICK("- rpt_call:  hanging up 'mychannel' and 'genchannel'");
		ast_hangup(mychannel);
		ast_hangup(genchannel);
		rpt_mutex_lock(&myrpt->lock);
		myrpt->callmode = 0;
		...
		if((!myrpt->patchquiet) && aborted) {
			DEBUG_QUICK("** Calling: rpt_telemetry(myrpt, TERM, NULL);");
	...
	{
		/* first put the channel on the conference in announce mode */
		DEBUG_QUICK("- rpt_call:  putting channel 'myrpt->pchannel->fds[0]' into conference mode");
		...
		DEBUG_QUICK("- rpt_call:  put vox monitoring on the channel");
	...
	{
		DEBUG_QUICK("- rpt_call:  while(myrpt->callmode)");
	...
	tone_zone_play_tone(genchannel->fds[0],-1);
	if (mychannel->pbx) {
		DEBUG_QUICK("- rpt_call:  softhangup: mychannel");
		ast_softhangup(mychannel,AST_SOFTHANGUP_DEV);
	}
	DEBUG_QUICK("- rpt_call:  hanging up 'genchannel'");
	ast_hangup(genchannel);
	rpt_mutex_lock(&myrpt->lock);
	myrpt->callmode = 0;
	myrpt->macropatch=0;
...
struct	rpt_link *l;
	...
	DEBUG_STATENTER("send_tele_link");
	...
	/* give it to everyone */
	while(l != &myrpt->links)
	{	DEBUG_LOGGER(LOG_NOTICE, "wf.data.ptr = str / str = %s\n", str);
		wf.data.ptr = str;
		if (l->chan && (l->mode == 1)) {
			DEBUG_QUICK("rpt_qwrite(l,&wf)");
			rpt_qwrite(l,&wf);
		}
		l = l->next;
	}                                                                                                                
	DEBUG_QUICK("rpt_telemetry(myrpt,VARCMD,cmd)");
	rpt_telemetry(myrpt,VARCMD,cmd);
	DEBUG_STATEXIT("send_tele_link");
	return;
}
...
static int connect_link(struct rpt *myrpt, char* node, int mode, int perma)
{
	...
	{
		DEBUG_QUICK("tlb_node_get(node,'n',NULL,NULL,NULL,NULL) == 1");
		sprintf(tmp,"tlb/%s/%s",node,myrpt->name);
		DEBUG_VAR_DATA(tmp);
	}
	else
	{	
		DEBUG_QUICK("!! Not a tlb node");
		if (node[0] != '3')
		{
			if (!node_lookup(myrpt,node,tmp,sizeof(tmp) - 1,1))
			{
				DEBUG_QUICK("!node_lookup(myrpt,node,tmp,sizeof(tmp) - 1,1)");
				if(strlen(node) >= myrpt->longestnode) {
					DEBUG_QUICK("no such node");
					return -1; /* No such node */
				}
				DEBUG_QUICK("No match yet");
				return 1; /* No match yet */
			}
		}
		else
		{
			DEBUG_QUICK("node[0] != '3'");
			char str1[60],str2[50];

			
			if (strlen(node) < 7) {
				DEBUG_QUICK("node less than 7 digits!");
				return 1;
			}
	...
	if(!strcmp(myrpt->name,node)) {/* Do not allow connections to self */
		DEBUG_QUICK("Do not allow connections to self");
		return -2;
	}
	...
	if (strncasecmp(tmp,"tlb",3)) /* if not tlb */
	{
			...
			strncasecmp(s1,"echolink/",9))
		{
			...
			*sy = 0;
			...
			s1 = sx;
			DEBUG_LOGGER(LOG_NOTICE, "s1 = %s", s1);
		}
		s2 = strsep(&s,",");
		DEBUG_VAR_DATA(s1);
		DEBUG_VAR_DATA(s2);
	...
	/* if found matching string */
		if (!strcmp(l->name, node)) {
			DEBUG_QUICK("found matching string");
			break;
		}
		DEBUG_QUICK("--- next node");
		l = l->next;
	}
	/* if found */
	if (l != &myrpt->links){
		DEBUG_QUICK("found");
		DEBUG_LOGGER(LOG_NOTICE, "l->mode = %d / mode = %d\n", l->mode, mode);
	/* if already in this mode, just ignore */
		if ((l->mode == mode) || (!l->chan)) {
			rpt_mutex_unlock(&myrpt->lock);
			DEBUG_QUICK("Already linked");
			return 2; /* Already linked */
		}
		if ((!strncasecmp(l->chan->name,"echolink",8)) ||
			(!strncasecmp(l->chan->name,"tlb",3)))
		{
			DEBUG_QUICK("Not echolnk or tlb channel");
			l->mode = mode;
			strncpy(myrpt->lastlinknode,node,MAXNODESTR - 1);
			DEBUG_LOGGER(LOG_NOTICE, "l->mode = %d / mode = %d, myrpt->lastlinknode = %s\n", l->mode, mode, myrpt->lastlinknode);
			rpt_mutex_unlock(&myrpt->lock);
			return 0;
		}
	...
	DEBUG_QUICK("zero the silly thing");
	memset((char *)l,0,sizeof(struct rpt_link));
	DEBUG_LOGGER(LOG_NOTICE, "mode = %d\n", mode);
	l->mode = mode;
	l->outbound = 1;
	l->thisconnected = 0;
	DEBUG_QUICK("voxinit_link");
	voxinit_link(l,1);
	strncpy(l->name, node, MAXNODESTR - 1);
	l->isremote = (s && ast_true(s));
	DEBUG_VAR_DATA(l->isremote);
	if (modechange) {
		DEBUG_QUICK("l->connected = 1");
		l->connected = 1;
	}
	l->hasconnected = l->perma = perma;
	DEBUG_LOGGER(LOG_NOTICE, "perma = %d / l->hasconnected = %d / l->perma = %d\n", perma, l->hasconnected, l->perma);
	...
	*tele++ = 0;
	if (!strncasecmp(deststr,"echolink",8))
	{
		...
		strcpy(cp,node + 1);
		DEBUG_LOGGER(LOG_NOTICE, "-----> Requesting channel of deststr = %s\n", deststr);
		l->chan = ast_request(deststr, AST_FORMAT_SLINEAR,  NULL, tel1,NULL);
	}
	else
	{
		DEBUG_LOGGER(LOG_NOTICE, "-----> Requesting channel of deststr = %s\n", deststr);
		...
		if (!(l->chan))
		{
			DEBUG_QUICK("!(l->chan), doing usleep(150000)");
			usleep(150000);
			DEBUG_LOGGER(LOG_NOTICE, "-----> Requesting channel of deststr = %s\n", deststr);
			l->chan = ast_request(deststr, AST_FORMAT_SLINEAR,NULL,tele,NULL);
		}
	...
	if (l->chan){
		DEBUG_QUICK("Channel seems to be up");
		DEBUG_QUICK("ast_set_read_format(l->chan, AST_FORMAT_SLINEAR);");
		ast_set_read_format(l->chan, AST_FORMAT_SLINEAR);
		DEBUG_QUICK("ast_set_write_format(l->chan, AST_FORMAT_SLINEAR);");
		...
		l->chan->whentohangup = 0;
                ...
                DEBUG_LOGGER(LOG_NOTICE, "rpt (remote) initiating call to %s/%s on %s\n",  deststr, tele, l->chan->name);
                if (debug > 3)
                        ast_log(LOG_NOTICE, "rpt (remote) initiating call to %s/%s on %s\n",
                deststr, tele, l->chan->name);
                if(l->chan->caller.id.number.valid)
                        ast_free(l->chan->caller.id.number.str);
                l->chan->caller.id.number.str = ast_strdup(myrpt->name);
                l->chan->caller.id.number.valid = 1;
                if (l->chan->connected.id.number.valid)
                        ast_free(l->chan->connected.id.number.str);
                l->chan->connected.id.number.str = ast_strdup(myrpt->name);
                l->chan->connected.id.number.valid = 1;
                DEBUG_QUICK("* performing ast_call");
                DEBUG_LOGGER(LOG_NOTICE, "tele = %s\n", tele);
                res = ast_call(l->chan,tele,0);
                DEBUG_LOGGER(LOG_NOTICE, "ast_call returned %d\n", res);
	}
	else
	{
		DEBUG_LOGGER(LOG_NOTICE, "Unable to place call to %s/%s\n",
				deststr,tele);
		if(debug > 3) 
	...
	/* allocate a pseudo-channel thru asterisk */
	DEBUG_QUICK("*) Attempting to allocate a pseudo channel");
	...
	DEBUG_QUICK(")) ast_set_read_format(l->pchan, AST_FORMAT_SLINEAR);");
	ast_set_read_format(l->pchan, AST_FORMAT_SLINEAR);
	DEBUG_QUICK(")) ast_set_write_format(l->pchan, AST_FORMAT_SLINEAR);");
	...
	DEBUG_QUICK("** first put the channel on the conference in proper mode");
	...
	rpt_mutex_lock(&myrpt->lock);
	if (tlb_node_get(node,'n',NULL,NULL,NULL,NULL) == 1) {
		DEBUG_QUICK("-- init_linkmode for tlb");
		init_linkmode(myrpt,l,LINKMODE_TLB);
	}
	else if (node[0] == '3') {
		DEBUG_QUICK("-- init_linkmode for echolink");
		init_linkmode(myrpt,l,LINKMODE_ECHOLINK);
	}
	else {
		DEBUG_QUICK("l->linkmode = 0");
		l->linkmode = 0;
	}
...
static int function_ilink(struct rpt *myrpt, char *param, char *digits, int command_source, struct rpt_link *mylink)
{
	...
	DEBUG_STATENTER("function_ilink");
	if(!param) {
		DEBUG_STATEXIT("function_ilink");
		return DC_ERROR;
	}
	...
	DEBUG_LOGGER(LOG_NOTICE, "@@@@ ilink param = %s, digitbuf = %s\n", (param)? param : "(null)", digitbuf);
	if(debug > 6)
	...
	switch(myatoi(param)){
		...
		case 1: /* Link off */
			DEBUG_QUICK("perm link off / link off");
			...
			if (l != &myrpt->links){ /* if found */
				...
				if ((myatoi(param) < 10) && 
				    (l->max_retries > MAX_RETRIES))
				{
					DEBUG_QUICK("(myatoi(param) < 10) && (l->max_retries > MAX_RETRIES)");
					rpt_mutex_unlock(&myrpt->lock);
					return DC_COMPLETE;
				}
				...
				l->retries = l->max_retries + 1;
				l->disced = 1;
				DEBUG_QUICK("^^^^ setting l->hasconnected = 1");
				l->hasconnected = 1;
				rpt_mutex_unlock(&myrpt->lock);
				...
				DEBUG_QUICK("rpt_telem_select(myrpt,command_source,mylink);");
				rpt_telem_select(myrpt,command_source,mylink);
				DEBUG_QUICK("rpt_telemetry(myrpt, COMPLETE, NULL);");
		...
		case 18: /* Link Monitor Local Only permanent */
			DEBUG_QUICK("Link Monitor/ Link Transceive/ Link Permanent Monitor/ Link Transcieve Permanent / Link Monitor Only / Link Monitor Only Permanent");
			if ((digitbuf[0] == '0') && (myrpt->lastlinknode[0]))
				strcpy(digitbuf,myrpt->lastlinknode);
			r = atoi(param);
			/* Attempt connection  */
			...
			mode = (r & 1) ? 1 : 0;
			if ((r == 8) || (r == 18)) mode = 2;
			DEBUG_QUICK("**> attempt connection");
			DEBUG_LOGGER(LOG_NOTICE, "digitbuf = %s / mode = %d / perma = %d\n", digitbuf, mode, perma);
			r = connect_link(myrpt, digitbuf, mode, perma);
			DEBUG_LOGGER(LOG_NOTICE, "** r = %d\n", r);
			switch(r){
				case -2: /* Attempt to connect to self */
					DEBUG_QUICK("-2 - attempt to connect to self");
					return DC_COMPLETE; /* Silent error */

				case 0:
					DEBUG_QUICK("0 - COMPLETE");
					myrpt->linkactivityflag = 1;
					rpt_telem_select(myrpt,command_source,mylink);
					rpt_telemetry(myrpt, COMPLETE, NULL);
					return DC_COMPLETE;
				...
				case 1:
					DEBUG_QUICK("1 - nothing");
					break;
				
				case 2:
					DEBUG_QUICK("2 - REMALREADY / COMPLETE");
					rpt_telem_select(myrpt,command_source,mylink);
					rpt_telemetry(myrpt, REMALREADY, NULL);
					return DC_COMPLETE;
				
				default:
					DEBUG_QUICK("default - CONNFAIL");
					rpt_telem_select(myrpt,command_source,mylink);
		...
		case 4: /* Enter Command Mode */
			DEBUG_QUICK("Enter Command Mode");
			if (strlen(digitbuf) < 1) break;
		...
		case 6: /* All Links Off, including permalinks */
			DEBUG_QUICK("All Links Off, including permalinks");
                       rpt_mutex_lock(&myrpt->lock);
		...
		case 10: /* All RANGER Links Off */
			DEBUG_QUICK("All RANGER Links Off");
                       rpt_mutex_lock(&myrpt->lock);
			myrpt->savednodes[0] = 0;
                        l = myrpt->links.next;
                        /* loop through all links */
		...
		case 7: /* Identify last node which keyed us up */
			DEBUG_QUICK("Identify last node which keyed us up");
			rpt_telem_select(myrpt,command_source,mylink);
		...
		case 16: /* Restore links disconnected with "disconnect all links" command */
			DEBUG_QUICK("Restore links disconnected with 'disconnect all links' command");
		...
		default:
			DEBUG_QUICK("Error: default");
			return DC_ERROR;
			
	}
	
	DEBUG_QUICK("Indeterminate");
	...
	switch(myatoi(argv[0])){
		...
		case 56: /* RX CTCSS Enable */
			    ...
			    (strncasecmp(myrpt->rxchannel->name,"dahdi/", 6) == 0))
			{
				...
				r.data = 0;
				ioctl(myrpt->dahdirxchannel->fds[0],DAHDI_RADIO_SETPARAM,&r);
			}
			if ((strncasecmp(myrpt->rxchannel->name,"radio/", 6) == 0) || 
	   ...
	   (!strcmp(myrpt->remoterig, remote_rig_kenwood)))
	{
		for(i = 0; i < txbytes - 1; i++)
		{
		        ...
		        prm.data = 0;
		       	prm.buf[0] = txbuf[i];
		        prm.index = 1;
			if (ioctl(myrpt->dahdirxchannel->fds[0],
...
static int attempt_reconnect(struct rpt *myrpt, struct rpt_link *l)
{
	...
	{
		ast_log(LOG_NOTICE,"attempt_reconnect: cannot find node %s\n",l->name);
		return -1;
	}
	...
	tele = strchr(deststr, '/');
	if (!tele) {
		ast_log(LOG_NOTICE,"attempt_reconnect:Dial number (%s) must be in format tech/number\n",deststr);
		return -1;
	}
	*tele++ = 0;
	l->elaptime = 0;
...
int ms = MSWAIT,i,lasttx=0,lastexttx = 0,lastpatchup = 0,val,identqueued,othertelemqueued, res = 0;
...
char tmpstr[300],lstr[MAXLINKLIST],lat[100],lon[100],elev[100];
	...
	*tele++ = 0;
	...
	if (myrpt->rxchannel)
	{
		...
		if (option_verbose > 2)
			DEBUG_LOGGER(LOG_NOTICE, "rpt (Rx) initiating call to %s/%s on %s\n",
				tmpstr,tele,myrpt->rxchannel->name);
			ast_verbose(VERBOSE_PREFIX_3 "rpt (Rx) initiating call to %s/%s on %s\n",
				tmpstr,tele,myrpt->rxchannel->name);
		res = ast_call(myrpt->rxchannel,tele,999);
		DEBUG_LOGGER(LOG_NOTICE, "%s / %s - ast_call returned\n", myrpt->rxchannel->appl, myrpt->rxchannel->data, res);
	...
	if (myrpt->txchanname)
	{
		...
		*tele++ = 0;
		...
		if (myrpt->txchannel)
		{
			...
			if (option_verbose > 2)
				DEBUG_LOGGER(LOG_NOTICE, "rpt (Tx) initiating call to %s/%s on %s\n",
					tmpstr,tele,myrpt->txchannel->name);
				ast_verbose(VERBOSE_PREFIX_3 "rpt (Tx) initiating call to %s/%s on %s\n",
					tmpstr,tele,myrpt->txchannel->name);
			res = ast_call(myrpt->txchannel,tele,999);
			DEBUG_LOGGER(LOG_NOTICE, "%s / %s - ast_call returned\n", myrpt->rxchannel->appl, myrpt->rxchannel->data, res);
			if (myrpt->rxchannel->_state != AST_STATE_UP)
			{
				rpt_mutex_unlock(&myrpt->lock);
	...
	while (ms >= 0)
	{
		...
		while(l != &myrpt->links)
		{
			...
			if (l->newkey == 1)
			{
				...
				{
					if (debug == 7) ast_log(LOG_DEBUG, "@@@@ rx un-key\n");
					l->lastrealrx = 0;
					l->rerxtimer = 0;
					if (l->lastrx1)
					{
				...
				(l->retries >= l->max_retries))
			{
				...
				{
					if (!l->hasconnected) {
						DEBUG_QUICK("(-)(-)(-) Link has disconnected! Reporting CONNFAIL");
						rpt_telemetry(myrpt,CONNFAIL,l);
					}
					else {
						DEBUG_QUICK("(_)(_)(_) Reporting REMDISC");
						rpt_telemetry(myrpt,REMDISC,l);
					}
				}
				DEBUG_QUICK("-=-=-=-=-=-=-=-=-=-=-");
				if (l->hasconnected) {
					DEBUG_QUICK("l->hasconnected");
					rpt_update_links(myrpt);
				}
				if (myrpt->p.archivedir)
				{
				...
				/* hang-up on call to device */
				DEBUG_QUICK(") hang-up on call to device");
				ast_hangup(l->pchan);
				ast_free(l);
                                rpt_mutex_lock(&myrpt->lock);
				break;
		...
		{
			//DEBUG_QUICK("if it was a read from rx");
			int ismuted;

			f = ast_read(myrpt->rxchannel);
			if (!f)
			{
				if (debug) ast_log(LOG_DEBUG, "@@@@ rpt:Hung Up\n");
				break;
			}
			if (f->frametype == AST_FRAME_VOICE)
			{
				...
				int n;
#endif
				//DEBUG_QUICK("f->frametype == AST_FRAME_VOICE");
				if (myrpt->p.rxburstfreq)
				{
					DEBUG_QUICK("myprt->p.rxburstfreq");
				...
				if (f1)
				{
					...
					{
						int bs;
						bs = write(myrpt->outstreampipe[1],f1->data.ptr,f1->datalen);
					}
				}
			}
#ifndef	OLD_ASTERISK
			...
			{
				DEBUG_QUICK("f->frametype == AST_FRAME_DTMF_BEGIN");
				if (myrpt->lastf1)
					memset(myrpt->lastf1->data.ptr,0,myrpt->lastf1->datalen);
				if (myrpt->lastf2)
					memset(myrpt->lastf2->data.ptr,0,myrpt->lastf2->datalen);
			...
			{
				DEBUG_QUICK("f->frametype == AST_FRAME_DTMF");
			...
			{
				DEBUG_QUICK("f->frametype == AST_FRAME_CONTROL");
				if (f->subclass.integer == AST_CONTROL_HANGUP)
				{
					DEBUG_QUICK("f->subclass.integer == AST_CONTROL_HANGUP");
					if (debug) ast_log(LOG_DEBUG, "@@@@ rpt:Hung Up\n");
					ast_frfree(f);
					break;
				}
				...
				{
					DEBUG_QUICK("f->subclass.integer == AST_CONTROL_RADIO_KEY");
				...
				{
					DEBUG_QUICK("f->subclass.integer == AST_CONTROL_RADIO_UNKEY");
			...
			{
				DEBUG_QUICK("f->frametype == AST_FRAME_TEXT");
				char buf[100];
				int j;
				...
				{
					DEBUG_QUICK("is a BeagleBoard device");
					/* if message parsable */
					if (sscanf(f->data.ptr,"GPIO%d %d",&i,&j) >= 2)
					{
		...
		{
			//DEBUG_QUICK("## if it was a read from pseudo-tx");
		...
		while(l != &myrpt->links)
		{
				...
				(ast_tvdiff_ms(now,l->lastlinktv) >= 19))
			{
				...
				if (l->phonemode == 0 && l->chan && (l->lasttx != totx))
				{
					if (totx)
					{
						DEBUG_QUICK("AST_CONTROL_RADIO_KEY");
						if (l->newkey < 2) ast_indicate(l->chan,AST_CONTROL_RADIO_KEY);
					}
					else
					{
						DEBUG_QUICK("AST_CONTROL_RADIO_UNKEY");
						ast_indicate(l->chan,AST_CONTROL_RADIO_UNKEY);
					}
					if (myrpt->p.archivedir)
					{
			...
			if (who == l->chan) /* if it was a read from rx */
			{
				...
				if (!f)
				{
					    ...
					    strncasecmp(l->chan->name,"tlb",3))
					{
						...
						{
							DEBUG_QUICK("(!l->disced) && (!l->outbound)");
							if ((l->name[0] <= '0') || (l->name[0] > '9') || l->isremote)
								l->disctime = 1;
							else
								l->disctime = DISC_TIME;
							rpt_mutex_lock(&myrpt->lock);
							DEBUG_QUICK("* performing ast_hangup(->chan)");
							ast_hangup(l->chan);
							l->chan = 0;
							break;
						}
						...
						if (l->retrytimer) 
						{
							DEBUG_QUICK("l->retrytimer");
							DEBUG_QUICK("* performing ast_hangup(l->chan");
							ast_hangup(l->chan);
							l->chan = 0;
							rpt_mutex_lock(&myrpt->lock);
							break; 
						...
						{
							DEBUG_QUICK("l->outbound && (l->retries++ < l->max_retries) && (l->hasconnected)");
							rpt_mutex_lock(&myrpt->lock);
							if (l->chan) {
								DEBUG_QUICK("* performing ast_hangup(->chan");
								ast_hangup(l->chan);
					...
					rpt_update_links(myrpt);
					if (!l->hasconnected) {
						DEBUG_QUICK(" !l->hasconnected, reporting CONNFAIL");
						rpt_telemetry(myrpt,CONNFAIL,l);
					}
					else if (l->disced != 2){
						DEBUG_QUICK("Reporting REMDISC");
					...
					if (l->hasconnected) {
						DEBUG_QUICK("* performing rpt_update_links(myrpt)");
						rpt_update_links(myrpt);
					}
					if (myrpt->p.archivedir)
					{
					...
					l->lastf2 = NULL;
					/* hang-up on call to device */
					DEBUG_QUICK("* hangup on call to device");
					ast_hangup(l->chan);
					ast_hangup(l->pchan);
					ast_free(l);
					rpt_mutex_lock(&myrpt->lock);
				...
				if (f->frametype == AST_FRAME_VOICE)
				{
					       ...
					       (!strncasecmp(l->chan->name,"tlb",3)))
					{
						...
						if (l->phonevox)
						{
							DEBUG_QUICK("l->phonevox");
							n1 = dovox(&l->vox,
								f->data.ptr,f->datalen / 2);
							if (n1 != l->wasvox)
							{
								DEBUG_LOGGER(LOG_NOTICE, "Link Node %s, vox %d\n",l->name,n1);
				...
				{
					DEBUG_QUICK("f->frametype == AST_DTMF_FRAME");
					if (l->lastf1)
						memset(l->lastf1->data.ptr,0,l->lastf1->datalen);
					if (l->lastf2)
						memset(l->lastf2->data.ptr,0,l->lastf2->datalen);
				...
				if (f->frametype == AST_FRAME_CONTROL)
				{
					...
					{
						DEBUG_QUICK("f->subclass.integer == AST_CONTROL_ANSWER");
						char lconnected = l->connected;
						DEBUG_VAR_DATA(lconnected);

						__kickshort(myrpt);
						myrpt->rxlingertimer = ((myrpt->iaxkey) ? RX_LINGER_TIME_IAXKEY : RX_LINGER_TIME);
						l->connected = 1;
						DEBUG_QUICK("^^^^^^^^^^^ settings l->hasconnected = 1");
						l->hasconnected = 1;
						l->thisconnected = 1;
						l->elaptime = -1;
						if (!l->phonemode) send_newkey(l->chan);
						if (!l->isremote) l->retries = 0;
						// change back to !lconnected to revert back
						if (!lconnected) 
						{
							DEBUG_QUICK(" * Reporting CONNECTED");
					...
					{
						DEBUG_QUICK("(f->subclass.integer == AST_CONTROL_RADIO_KEY) && (l->newkey < 2) /  RX key");
						if (debug == 7 ) ast_log(LOG_DEBUG, "@@@@ rx key\n");
						l->lastrealrx = 1;
						l->rerxtimer = 0;
						if (!l->lastrx1)
						{
					...
					if (f->subclass.integer == AST_CONTROL_RADIO_UNKEY)
					{
						...
						if (l->lastrx1)
						{
							...
							rpt_update_links(myrpt);
							if(myrpt->p.duplex) {
								DEBUG_QUICK("* Reporting LINKUNKEY");
								rpt_telemetry(myrpt,LINKUNKEY,l);
							}
						}
					}
					if (f->subclass.integer == AST_CONTROL_HANGUP)
					{
							...
							strncasecmp(l->chan->name,"tlb",3))
						{
							if ((!l->outbound) && (!l->disced))
							{
								if ((l->name[0] <= '0') || (l->name[0] > '9') || l->isremote)
									...
									l->disctime = DISC_TIME;
								rpt_mutex_lock(&myrpt->lock);
								DEBUG_QUICK(" Hanging up channel");
								ast_hangup(l->chan);
								l->chan = 0;
								break;
							}
						...
						rpt_mutex_unlock(&myrpt->lock);
						if (!l->hasconnected) {
							DEBUG_QUICK("* Reporting CONNFAIL");
							rpt_telemetry(myrpt,CONNFAIL,l);
						}
						else if (l->disced != 2) {
							DEBUG_QUICK("* Reporting REMDISC");
						...
						if (l->hasconnected) {
							DEBUG_QUICK("* Performign rpt_update_links");
							rpt_update_links(myrpt);
						}
						if (myrpt->p.archivedir)
						{
		...
		if (who == myrpt->monchannel) 
		{
			...
			if (f->frametype == AST_FRAME_VOICE)
			{
				...
				{
					int bs;
					bs = write(myrpt->outstreampipe[1],f->data.ptr,f->datalen);
				}
				fs = ast_frdup(f);
				fac = 1.0;
				if (l->chan && (!strncasecmp(l->chan->name,"echolink",8)))
	...
	DEBUG_QUICK("ast_hangup(myrpt->pchannel)");
	ast_hangup(myrpt->pchannel);
	DEBUG_QUICK("ast_hangup(myrpt->monchannel)");
	ast_hangup(myrpt->monchannel);
	if (myrpt->parrotchannel) {
		DEBUG_QUICK("ast_hangup(myrpt->parrotchannel)");
		ast_hangup(myrpt->parrotchannel);
	}
	myrpt->parrotstate = 0;
	if (myrpt->voxchannel) {
		DEBUG_QUICK("ast_hangup(myrpt->voxchannel)");
		ast_hangup(myrpt->voxchannel);
	}
	DEBUG_QUICK("ast_hangup(myrpt->txpchannel)");
	ast_hangup(myrpt->txpchannel);
	if (myrpt->txchannel != myrpt->rxchannel) {
		DEBUG_QUICK("ast_hangup(myrpt->txchannel)");
		ast_hangup(myrpt->txchannel);
	}
	if (myrpt->dahditxchannel != myrpt->txchannel) {
		DEBUG_QUICK("ast_hangup(myrpt->dahditxchannel)");
	...
	myrpt->lastf2 = NULL;
	DEBUG_QUICK("ast_hangup(myrpt->rxchannel)");
	ast_hangup(myrpt->rxchannel);
	rpt_mutex_lock(&myrpt->lock);
	l = myrpt->links.next;
	while(l != &myrpt->links)
	{
		...
		/* hang-up on call to device */
		if (l->chan) {
			DEBUG_QUICK("ast_hangup(l->chan)");
			ast_hangup(l->chan);
		}
		DEBUG_QUICK("ast_hangup(l->pchan)");
		ast_hangup(l->pchan);
		l = l->next;
		ast_free(ll);
	}
	...
	rpt_mutex_unlock(&myrpt->lock);
	if (debug) ast_log(LOG_DEBUG, "@@@@ rpt:Hung up channel\n");
	myrpt->rpt_thread = AST_PTHREADT_STOP;
	if (myrpt->outstreampid) {
		DEBUG_QUICK(" * Killing myrpt->outstreampid with a SIGTERM");
...
char *this,*val;
	...
	/* wait until asterisk starts */
        while(!ast_test_flag(&ast_options,AST_OPT_FLAG_FULLY_BOOTED)) {
                usleep(250000);
                DEBUG_LOGGER(LOG_NOTICE, "** Not Fully Booted!  Waiting....\n");
        }
#endif
#ifdef	NEW_ASTERISK
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, " (this = ast_category_browse(cfg,this)) != NULL\n");
		for(i = 0 ; i < strlen(this) ; i++){
			if((this[i] < '0') || (this[i] > '9'))
				break;
		}
		if (i != strlen(this)) {
			DEBUG_LOGGER(LOG_NOTICE, "Not a node denf/n");
		...
		if (val) 
		{
			DEBUG_LOGGER(LOG_NOTICE, "- (remote) val is true\n");
			rpt_vars[n].remoterig = ast_strdup(val);
			rpt_vars[n].remote = 1;
		}
		val = (char *) ast_variable_retrieve(cfg,this,"radiotype");
		if (val) {
			DEBUG_LOGGER(LOG_NOTICE, "- (radiotype) val is true\n");
			rpt_vars[n].remoterig = ast_strdup(val);
		}
	...
	for(i = 0; i < n; i++)
	{
		DEBUG_LOGGER(LOG_NOTICE, "- load_rpt_vars(i,1) : i = %d\n", i);
		load_rpt_vars(i,1);

		/* if is a remote, dont start one for it */
		if (rpt_vars[i].remote)
		{
			DEBUG_LOGGER(LOG_NOTICE, "- it is a remote\n");
		...
		{
		    DEBUG_LOGGER(LOG_NOTICE, "- This is a normal repeater\n");
	...
	DEBUG_LOGGER(LOG_NOTICE, "** rpt_master entering thread monitor mode.\n");
	for(;;)
	{
	...
	DEBUG_LOGGER(LOG_NOTICE, "** rpt_master exiting \n");
	ast_mutex_unlock(&rpt_master_lock);
	ast_config_destroy(cfg);
	pthread_exit(NULL);
}
...
{
	int res = 0; 
	int i,x,rem_totx,rem_rx,remkeyed,n,phone_mode = 0;
	...
	DEBUG_STATENTER("rpt_exec");	

	/* With Asterisk 1.8, we can no longer use | as the delimeter (it has changed to ,).  To make things easier on
ourselves, we are using the built-in argument parser to give us what is passed to Rpt.
*/

	AST_DECLARE_APP_ARGS(args,
		AST_APP_ARG(opts)[100];
	);
	...
	DEBUG_VAR_DATA(parse);
	
	AST_STANDARD_APP_ARGS(args, parse);
	
	struct ast_str *tbuf = ast_str_create(256);
	
	if (!tbuf) {
		ast_log(LOG_ERROR, "Unable to allocate parse buffer\n");
		ast_free(tbuf);
		return -1;
	}
	
	
	strcpy(tmp, args.opts[0]);
	...
	DEBUG_LOGGER(LOG_NOTICE, "Number of args: %d\n", args.argc);
	DEBUG_LOGGER(LOG_NOTICE, "Node number: %s\n", args.opts[0]);

	for (i = 1; i < args.argc; i++) {
		bflg=0;

		DEBUG_LOGGER(LOG_NOTICE,"Argument %d is %s \n", i, args.opts[i]);

		altp=strstr(args.opts[i], "|*");
		if(altp) {
			altp[0]=0;
			altp++;
			DEBUG_VAR_DATA(altp);
	...
	if (myrpt == NULL)
	{
		...
		val = NULL;
		myadr = NULL;
		b1 = S_COR(chan->caller.id.number.valid, chan->caller.id.number.str, NULL);		
		if (b1) {
			DEBUG_VAR_DATA(b1);
		...
		{
			DEBUG_LOGGER(LOG_NOTICE, "Info- no options or X/F: %s\n", options);

			myadr = (char *) ast_variable_retrieve(cfg, "proxy", "ipaddr");
			if (options && (*options == 'F'))
			{
				DEBUG_LOGGER(LOG_NOTICE, "-> option is F\n");
				if (b1 && myadr) 
				{
					DEBUG_LOGGER(LOG_NOTICE, "Have valid number and ip address: %s / %s", b1, myadr);
			...
			{
				DEBUG_LOGGER(LOG_NOTICE, "* forward_node_lookup: %s / %s / %s\n", myrpt,tmp,cfg); 
				val = forward_node_lookup(myrpt,tmp,cfg);
				DEBUG_VAR_DATA(val);
			}
		}
		if (b1 && val && myadr && cfg)
		{
			DEBUG_LOGGER(LOG_NOTICE, "b1: %s / val: %s / myadr: %s / cfg: %s\n", b1,val,myadr,cfg);
			strncpy(xstr,val,sizeof(xstr) - 1);
			if (!options)
			{
				DEBUG_LOGGER(LOG_NOTICE, "-> no options!\n");
				if (*b1 < '1')
				{
				...
				DEBUG_LOGGER(LOG_NOTICE, "Looking for reported node string- myrpt: %s / b1: %s / cfg: %s\n", myrpt,b1,cfg);
				val = forward_node_lookup(myrpt,b1,cfg);
				DEBUG_VAR_DATA(val);
				if (!val)
				{
			...
			if (!strcmp(myadr,s2)) /* if we have it.. */
			{
				...
				if (mypfx) {
					...
					DEBUG_LOGGER(LOG_NOTICE, "** radio-proxy@%s%s/%s\n",mypfx,tmp,tmp2);
				}
				else {
					snprintf(dstr,sizeof(dstr) - 1,"radio-proxy@%s/%s",tmp,tmp2);
					DEBUG_LOGGER(LOG_NOTICE, "** radio-proxy@%s/%s\n",tmp,tmp2);
				}
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "Option is q\n");
		int res=0;
	 	char buf2[128];
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "Options is V or v\n");
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "Options is o\n");
	...
	if((altp)&&(*options == 'Z'))
	{
		DEBUG_LOGGER(LOG_NOTICE, "Options is z\n");
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "Options is P/D/R/S: %s\n", options);
		int val;
		...
		{
			DEBUG_LOGGER(LOG_NOTICE, "myrpt->bargechan=1\n");
			myrpt->bargechan=1;		
		}

		if(memp>0)
		{
			DEBUG_LOGGER(LOG_NOTICE, "memp>0\n");
			char radiochan;
			...
			{
				DEBUG_QUICK("channel_steer(myrpt,memp)");
		...
		DEBUG_QUICK("ast_set_callerid");
		ast_set_callerid(chan,"0","app_rpt user","0");
		val = 1;
		DEBUG_QUICK("ast_channel_setoption");
		...
		DEBUG_VAR_DATA(phone_vox, phone_monitor);
	}
	else
	{
		ast_log(LOG_NOTICE, "***** Incoming call is type %s *****\n", chan->name);
#ifdef ALLOW_LOCAL_CHANNELS
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "Options is R\n");
		/* Parts of this section taken from app_parkandannounce */
		char *return_context;
		...
		{
			DEBUG_LOGGER(LOG_NOTICE, "!!myrpt->p.nobusyout) && m\n");
			if (chan->_state != AST_STATE_UP)
			{
				ast_indicate(chan,AST_CONTROL_BUSY);
				DEBUG_LOGGER(LOG_NOTICE, "ast_indicate: chan = AST_CONTROL_BUSY\n");
			}
		...
		{
			ast_answer(chan);
			DEBUG_LOGGER(LOG_NOTICE, "- Answering channel\n");
		...
		if(s) {
			DEBUG_LOGGER(LOG_NOTICE, "- s is set, updating timeout\n");
		...
		if(return_context != NULL) {
			...
			DEBUG_LOGGER(LOG_NOTICE, "return_context is not NULL\n");
			working = return_context;
			context = strsep(&working, "|");
			exten = strsep(&working, "|");
			if(!exten) {
				/* Only a priority in this one */
				DEBUG_LOGGER(LOG_NOTICE, "!exten\n");
				priority = context;
				exten = NULL;
				context = NULL;
			} else {
				...
				if(!priority) {
					DEBUG_LOGGER(LOG_NOTICE, "!priority\n");
		...
		if(exten)
		{
			...
			DEBUG_LOGGER(LOG_NOTICE, "BYEXTENSION\n");
		}
		if(context)
		{
			strncpy(chan->context, context, sizeof(chan->context)-1);
			DEBUG_LOGGER(LOG_NOTICE, "context is true\n");
		}
		} else {  /* increment the priority by default*/
			chan->priority++;
			DEBUG_LOGGER(LOG_NOTICE, "Incrementing chan->priority\n");
		}

		if(option_verbose > 2) {
			ast_verbose( VERBOSE_PREFIX_3 "Return Context: (%s,%s,%d) ID: %s\n", chan->context,chan->exten, chan->priority, chan->caller.id.number.str);
			if(!ast_exists_extension(chan, chan->context, chan->exten, chan->priority, chan->caller.id.number.str)) {
		...
		DEBUG_QUICK("** ast_masq_park_call");
		...
		DEBUG_QUICK("-> rpt_telemetry");
		DEBUG_VAR_DATA(myrpt, REV_PATCH, tmp);
		
		rpt_telemetry(myrpt,REV_PATCH,tmp);

		DEBUG_LOGGER(LOG_NOTICE, "-> ast_free(orig_s)\n");
		ast_free(orig_s);
	...
	if ((!options) && (!i))
	{
		DEBUG_LOGGER(LOG_NOTICE, "((!options) && (!i))\n");
	        struct ast_hostent ahp;
	        struct hostent *hp;
	        struct in_addr ia;
		...
		/* get his IP from IAX2 module */
		DEBUG_LOGGER(LOG_NOTICE, "-> Get his IP from IAX2 module\n");
		memset(hisip,0,sizeof(hisip));
#ifdef ALLOW_LOCAL_CHANNELS
	        /* set IP address if this is a local connection*/
	        if (strncmp(chan->name,"Local",5)==0) {
	...
	if (!myrpt->remote)
	{
		DEBUG_LOGGER(LOG_NOTICE, "-> !myrpt->remote: this is not a remote\n");
		char *b,*b1;
		int reconnects = 0;
		struct timeval now;
		...
		{
			DEBUG_LOGGER(LOG_NOTICE, "((!ast_tvzero(myrpt->lastlinktime)) && (ast_tvdiff_ms(now,myrpt->lastlinktime) < 250)/n");
			rpt_mutex_unlock(&myrpt->lock);
		...
		if (phone_mode) 
		{
			DEBUG_LOGGER(LOG_NOTICE, "-> phone_mode is true\n");
			b1 = "0";
			b = NULL;
		...
		{
			DEBUG_LOGGER(LOG_NOTICE, "-> phone_mode is false\n");
			b1 = ast_strdupa(b);
			ast_shrink_phone_number(b1);
			/* if is an IAX client */
		...
		{
			rpt_mutex_lock(&myrpt->lock);
			DEBUG_LOGGER(LOG_NOTICE, "!b1[i] -- not a call-based node number\n");
			l = myrpt->links.next;
			/* try to find this one in queue */
			DEBUG_LOGGER(LOG_NOTICE, "-> try to find this one in queue\n");
			while(l != &myrpt->links)
			{
				DEBUG_LOGGER(LOG_NOTICE, "l != &myrpt->links \n");
				if (l->name[0] == '0') 
				{
					DEBUG_LOGGER(LOG_NOTICE, "l->name[0] == '0'\n");
					l = l->next;
					continue;
				}
				/* if found matching string */
			...
			if (l != &myrpt->links) 
			{
				DEBUG_LOGGER(LOG_NOTICE, "-> if found\n");
				l->killme = 1;
				l->retries = l->max_retries + 1;
				l->disced = 2;
				reconnects = l->reconnects;
				...
				rpt_mutex_unlock(&myrpt->lock);
				DEBUG_LOGGER(LOG_NOTICE, "-> no found\n");
			}
		}
		...
		/* zero the silly thing */
		DEBUG_LOGGER(LOG_NOTICE, "- zero the silly thing\n");
		memset((char *)l,0,sizeof(struct rpt_link));
		l->mode = 1;
		...
		gettimeofday(&myrpt->lastlinktime,NULL);
		/* allocate a pseudo-channel thru asterisk */
		DEBUG_LOGGER(LOG_NOTICE, "* Attempting to allocate a pseudo-channel\n");
		l->pchan = ast_request("DAHDI", AST_FORMAT_SLINEAR, chan, "pseudo", NULL);
		if (!l->pchan)
		{
			DEBUG_LOGGER(LOG_NOTICE, "rpt:Sorry unable to obtain pseudo channel\n");
		...
		if (chan->_state != AST_STATE_UP)
		{
			DEBUG_LOGGER(LOG_NOTICE, "chan_state != AST_STATE_UP\n");
			ast_answer(chan);
			if (l->name[0] > '9')
			{
				DEBUG_LOGGER(LOG_NOTICE, "l->name[0] > '9'\n'");
			...
			else
			{
				if (!phone_mode) {
					send_newkey(chan);
					DEBUG_LOGGER(LOG_NOTICE, "-> !phone_mode\n");
				}
			}
		}
		rpt_update_links(myrpt);
		if (myrpt->p.archivedir)
		{
			DEBUG_LOGGER(LOG_NOTICE, "myrpt->p.archivedie\n");
			char str[100];

			if (l->phonemode) {
				sprintf(str,"LINK(P),%s",l->name);
				DEBUG_LOGGER(LOG_NOTICE, "l->phonemode\n");
			}
			else {
		...
		/* AST_PBX_KEEPALIVE */
		DEBUG_QUICK("*** AST_PBX_KEEPALIVE");
		return AST_PBX_KEEPALIVE;
	}
	/* well, then it is a remote */
	DEBUG_LOGGER(LOG_NOTICE, "** well, then this is a remote\n");
	rpt_mutex_lock(&myrpt->lock);
	/* look at callerid to see what node this comes from */
	b = S_COR(chan->caller.id.number.valid, chan->caller.id.number.str, NULL);
	DEBUG_VAR_DATA(b);
	...
	if ((b1[0] == '0') && b && b[0] && (strlen(b) <= 8)) {
		b1 = b;
		DEBUG_QUICK("-> If this is an IAX client");
	}
	if (b1 && (*b1 > '9')) myrpt->remote_webtransceiver = chan;
	/* if remote, error if anyone else already linked */
	if (myrpt->remoteon)
	{
		DEBUG_QUICK("-> If remote, error if anuone else alredy linked");
		rpt_mutex_unlock(&myrpt->lock);
		usleep(500000);
		if (myrpt->remoteon)
		{
			...
			{
				DEBUG_QUICK("-> myrpt->remote_webtransceiver || (b && (*b > '9'))");
	...
	rpt_mutex_unlock(&myrpt->lock);
	/* find our index, and load the vars initially */
	DEBUG_LOGGER(LOG_NOTICE, "-> find our index, and load the vars initally\n");
	for(i = 0; i < nrpts; i++)
	{
		if (&rpt_vars[i] == myrpt)
		{
	...
	if (!tele)
	{
		DEBUG_LOGGER(LOG_NOTICE, "rpt:Dial number must be in format tech number\n");
...
/* KG7QIN - break 1 */
	*tele++ = 0;
	myrpt->rxchannel = ast_request(myrpt->rxchanname,AST_FORMAT_SLINEAR,chan,tele,NULL);
	myrpt->dahdirxchannel = NULL;
	if (!strcasecmp(myrpt->rxchanname,"DAHDI")) {
		myrpt->dahdirxchannel = myrpt->rxchannel;
		DEBUG_LOGGER(LOG_NOTICE, "-> !strcasecmp(myrpt->rxchanname,'DAHDI'\n)");
	}
	if (myrpt->rxchannel)
	{
		DEBUG_LOGGER(LOG_NOTICE, "-> myrpt->rxchannel is true\n");
		ast_set_read_format(myrpt->rxchannel,AST_FORMAT_SLINEAR);
		ast_set_write_format(myrpt->rxchannel,AST_FORMAT_SLINEAR);
#ifdef	AST_CDR_FLAG_POST_DISABLED
		if (myrpt->rxchannel->cdr)
	...
	*--tele = '/';
	...
	if (myrpt->txchanname)
	{
		DEBUG_LOGGER(LOG_NOTICE, "-> myrpt->txchannel is true\n");
		tele = strchr(myrpt->txchanname,'/');
		if (!tele)
		{
			DEBUG_LOGGER(LOG_NOTICE,"-> !tele -- rpt:Dial number must be in format tech/number\n\n");
			fprintf(stderr,"rpt:Dial number must be in format tech/number\n");
			rpt_mutex_unlock(&myrpt->lock);
			ast_hangup(myrpt->rxchannel);
			pthread_exit(NULL);
		...
		*tele++ = 0;
		myrpt->txchannel = ast_request(myrpt->txchanname,AST_FORMAT_SLINEAR, chan, tele,NULL);
	...
	/* allocate a pseudo-channel thru asterisk */
	myrpt->pchannel = ast_request("DAHDI", AST_FORMAT_SLINEAR, chan, "pseudo", NULL);
	if (!myrpt->pchannel)
	{
		DEBUG_LOGGER(LOG_NOTICE, "-> rpt:Sorry unable to obtain pseudo channel\n");
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
	...
	if (myrpt->p.ioport && ((myrpt->iofd = openserial(myrpt,myrpt->p.ioport)) == -1))
	{
		DEBUG_LOGGER(LOG_NOTICE, "if serial io port, open it\n");
		rpt_mutex_unlock(&myrpt->lock);
		ast_hangup(myrpt->pchannel);
		if (myrpt->txchannel != myrpt->rxchannel) 
			ast_hangup(myrpt->txchannel);
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "- (myrpt->iofd < 1) && (myrpt->txchannel == myrpt->dahditxchannel)\n");
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "- myrpt->txchannel == myrpt->dahditxchannel\n");
		i = DAHDI_ONHOOK;
	...
	{
		/* go thru all the specs */
		DEBUG_LOGGER(LOG_NOTICE, "- go through all the specs\n");
		for(i = 0; i < myrpt->p.nlconn; i++)
		{
			...
			{
				sprintf(string,"GPIO %d %d",j,k);
				DEBUG_LOGGER(LOG_NOTICE, "string: %s\n", string);
				ast_sendtext(myrpt->rxchannel,string);
			}
	...
	if (myrpt->p.startupmacro)
	{
		DEBUG_LOGGER(LOG_NOTICE, "- myrpt->p.startupmacro\n");
		snprintf(myrpt->macrobuf,MAXMACRO - 1,"PPPP%s",myrpt->p.startupmacro);
	}
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "- if we are on 2w loop and are a remote, turn EC on\n");
		i = 128;
		ioctl(myrpt->dahdirxchannel->fds[0],DAHDI_ECHOCANCEL,&i);
	}
	if (chan->_state != AST_STATE_UP) {
		ast_answer(chan);
		DEBUG_LOGGER(LOG_NOTICE, "- chan->_state != AST_STATE_UP\n");
		if (!phone_mode)  {
			send_newkey(chan);
			DEBUG_LOGGER(LOG_NOTICE, "- !phone_mode\n");
		}
	...
	{
		DEBUG_LOGGER(LOG_NOTICE, "- myrpt->rxchannel == myrpt->dahdirxchannel\n");
		if (ioctl(myrpt->dahdirxchannel->fds[0],DAHDI_GET_PARAMS,&par) != -1)
		{
			DEBUG_LOGGER(LOG_NOTICE, "- ioctl(myrpt->dahdirxchannel->fds[0],DAHDI_GET_PARAMS,&par) != -1\n");
			if (par.rxisoffhook)
			{
				DEBUG_LOGGER(LOG_NOTICE, "- par.rxisoffhook is true\n");
	...
	{
		b1 = b;
		DEBUG_LOGGER(LOG_NOTICE, "- If is an IAX client\n");
	}
	if (myrpt->p.archivedir)
	{
		DEBUG_LOGGER(LOG_NOTICE, "myrpt->p.archivedir\n");
		...
		if (myrpt->p.monminblocks)
		{
			DEBUG_LOGGER(LOG_NOTICE, "- myrpt->p.monminblocks\n");
			blocksleft = diskavail(myrpt);
			if (myrpt->p.remotetimeout)
			{
				DEBUG_LOGGER(LOG_NOTICE, "- myrpt->p.remotetimeout\n");
			...
			if (blocksleft >= myrpt->p.monminblocks)
			{
				...
				DEBUG_LOGGER(LOG_NOTICE, "- blocksleft >= myrpt->p.monminblocks\n");
			}
		} else { 
			ast_cli_command(nullfd,mycmd);
			DEBUG_LOGGER(LOG_NOTICE, "!blocksleft >= myrpt->p.monminblocks\n");
	...
	if (myrpt->remote_webtransceiver) {
		myrpt->newkey = 2;
		DEBUG_LOGGER(LOG_NOTICE, "-> webtransciever active\n");
	}
	myrpt->loginuser[0] = 0;
	myrpt->loginlevel[0] = 0;
	...
	for(;;) 
	{
		DEBUG_LOGGER(LOG_NOTICE, "- START UN-LOCKED\n");
		if (ast_check_hangup(chan)) {
			DEBUG_LOGGER(LOG_NOTICE, "** chan hungup\n");
			break;
		}
		if (ast_check_hangup(myrpt->rxchannel)) {
			DEBUG_LOGGER(LOG_NOTICE, "** myrpt->rxchannel hungup\n");
			break;
		}
		notremming = 0;
		setting = 0;
		...
		while(telem != &myrpt->tele)
		{
			DEBUG_LOGGER(LOG_NOTICE, "-> while(telem != &myrpt->tele)	\n");
			if (telem->mode == SETREMOTE) {
				setting = 1;
				DEBUG_LOGGER(LOG_NOTICE, "-> telem->mode == SETREMOTE\n");
			}
			if ((telem->mode == SETREMOTE) ||
			    (telem->mode == SCAN) ||
				(telem->mode == TUNE))  {
					reming = 1;
					DEBUG_LOGGER(LOG_NOTICE, "-> reming = 1\n");
				}
			else {
				notremming = 1;
				DEBUG_LOGGER(LOG_NOTICE, "-> notreming = 1\n");
			}
			telem = telem->next;
		}
		if (myrpt->reload)
		{
			DEBUG_LOGGER(LOG_NOTICE, "myrpt->reload is true\n");
			myrpt->reload = 0;
			/* find our index, and load the vars */
			for(i = 0; i < nrpts; i++)
			{	
				...
				if (&rpt_vars[i] == myrpt)
				{
					DEBUG_LOGGER(LOG_NOTICE, "** Repeater found, loading vars: myrpt = %s\n", myrpt);
					load_rpt_vars(i,0);
					break;
				}
			}
		...
		if (myrpt->p.remotetimeout)
		{ 
			DEBUG_LOGGER(LOG_NOTICE, "myrpt->p.remote.timeout!\n");
			time_t r;

			r = (t - myrpt->start_time);
			if (r >= myrpt->p.remotetimeout)
			{
				DEBUG_LOGGER(LOG_NOTICE, "r >= myrpt->p.remotetimeout!\n");
...
/* KG7QIN - break 1 */
				...
				{
				    DEBUG_QUICK("myrpt->p.remotetimeoutwarningfreq is true");
				    if ((t - last_timeout_warning) >=
					myrpt->p.remotetimeoutwarningfreq)
				    {
				    	DEBUG_QUICK("(t - last_timeout_warning) >= myrpt->p.remotetimeoutwarningfreq");
					time(&last_timeout_warning);
					DEBUG_QUICK("rpt_telemetry(myrpt,TIMEOUT_WARNING,0)");
				...
				else
				{
				    ...
				    {
				    	DEBUG_QUICK("!last_timeout_warning");
					time(&last_timeout_warning);
					DEBUG_QUICK("rpt_telemetry(myrpt,TIMEOUT_WARNING,0);");
					rpt_telemetry(myrpt,TIMEOUT_WARNING,0);
				    }
		...
		{ 
			DEBUG_QUICK("myrpt->p.remoteinacttimeout && myrpt->last_activity_time");
			time_t r;

			r = (t - myrpt->last_activity_time);
			if (r >= myrpt->p.remoteinacttimeout)
			{
				DEBUG_QUICK("r >= myrpt->p.remoteinacttimeout");
				saynode(myrpt,chan,myrpt->name);
				DEBUG_QUICK("ast_safe_sleep(chan,1000);");
				ast_safe_sleep(chan,1000);
				break;
			}
			if ((myrpt->p.remotetimeoutwarning) && 
				...
				myrpt->p.remotetimeoutwarning)) &&
				    (r <= (myrpt->p.remoteinacttimeout - 
				    	myrpt->p.remotetimeoutwarningfreq)))
			{
				DEBUG_QUICK("timeout warning check 1");
				if (myrpt->p.remotetimeoutwarningfreq)
				{
				    DEBUG_QUICK("myrpt->p.remotetimeoutwarningfreq");
				    if ((t - last_timeout_warning) >=
					myrpt->p.remotetimeoutwarningfreq)
				    {
				    	DEBUG_QUICK("(t - last_timeout_warning) >= myrpt->p.remotetimeoutwarningfreq");
					time(&last_timeout_warning);
					DEBUG_QUICK("rpt_telemetry(myrpt,ACT_TIMEOUT_WARNING,0);");
		...
		if (who == NULL) {
			DEBUG_QUICK("who == NULL");
			ms = 0;
		}
		elap = MSWAIT - ms;
		if (myrpt->macrotimer) {
			DEBUG_QUICK("myrpt->macrotimer");
			myrpt->macrotimer -= elap;
		}
		if (myrpt->macrotimer < 0) {
			DEBUG_QUICK("myrpt->macrotimer < 0");
			myrpt->macrotimer = 0;
		}
		if (!ms) {
			DEBUG_QUICK("!ms");
		...
		if (myrpt->dtmf_local_timer)
		{
			DEBUG_QUICK("myrpt->dtmf_local_timer");
			if (myrpt->dtmf_local_timer > 1) {
				DEBUG_QUICK("myrpt->dtmf_local_timer > 1");
		...
		if (myrpt->voxtotimer) {
			DEBUG_QUICK("myrpt->voxtotimer");
			myrpt->voxtotimer -= elap;
		}
		if (myrpt->voxtotimer < 0) {
			DEBUG_QUICK("myrpt->voxtotimer < 0");
			myrpt->voxtotimer = 0;
		}
		myrx = keyed;
		if (phone_mode && phone_vox)
		{
			DEBUG_QUICK("phone_mode && phone_vox");
			...
			if (myrpt->voxtotimer <= 0)
			{
				...
				DEBUG_VAR_DATA(myrpt->voxtostate);
			}
			if (!myrpt->voxtostate) {
				DEBUG_QUICK("!myrpt->voxtostate");
		...
		if (myrpt->rxlingertimer) {
			DEBUG_QUICK("myrpt->rxlingertimer");
			myrpt->rxlingertimer -= elap;
		}
		if (myrpt->rxlingertimer < 0) {
			DEBUG_QUICK("myrpt->rxlingertimer < 0");
			myrpt->rxlingertimer = 0;
		}

		if ((myrpt->newkey == 2) && keyed && (!myrpt->rxlingertimer))
		{
			DEBUG_QUICK("(myrpt->newkey == 2) && keyed && (!myrpt->rxlingertimer)");
			myrpt->rerxtimer = 0;
			keyed = 0;
		}
		rpt_mutex_lock(&myrpt->lock);
		...
		if (!keyed && (keyed1))
		{
			...
			if ((myrpt->p.authlevel) && 
				...
				(myt > (t + 3)))
			{
				DEBUG_QUICK("if login necessaey, and not too soon");
				authreq = 1;
				authtold = 0;
		...
		{
			DEBUG_QUICK("rem_rx && (!myrpt->remoterx)");
			myrpt->remoterx = 1;
			if (myrpt->newkey < 2) {
				DEBUG_QUICK("myrpt->newkey < 2");
				ast_indicate(chan,AST_CONTROL_RADIO_KEY);
			}
		}
		if ((!rem_rx) && (myrpt->remoterx))
		...
		{	
			DEBUG_QUICK("authreq && (!myrpt->loginlevel[0])");
			if ((!authtold) && ((myrpt->authtelltimer += elap) 
				 >= AUTHTELLTIME))
			{
				authtold = 1;
				DEBUG_QUICK("rpt_telemetry(myrpt,LOGINREQ,NULL);");
				rpt_telemetry(myrpt,LOGINREQ,NULL);
			}
			if ((myrpt->authtimer += elap) >= AUTHLOGOUTTIME)
			{
				DEBUG_QUICK("if not logged in, hang up after a time");
				break; /* if not logged in, hang up after a time */
			}
		}
		if (myrpt->newkey == 1)
		{
			DEBUG_QUICK("myrpt->newkey == 1");
			if ((myrpt->retxtimer += elap) >= REDUNDANT_TX_TIME)
			{
				DEBUG_QUICK("(myrpt->retxtimer += elap) >= REDUNDANT_TX_TIME");
				myrpt->retxtimer = 0;
			...
			{
				DEBUG_QUICK("(myrpt->rerxtimer += elap) >= (REDUNDANT_TX_TIME * 2)");
				keyed = 0;
				myrpt->rerxtimer = 0;
			}
		}
		...
		{
			DEBUG_QUICK("rem_totx && (!myrpt->remotetx)");
			/* if not authed, and needed, do not transmit */
			if ((!myrpt->p.authlevel) || myrpt->loginlevel[0])
			{
				DEBUG_QUICK("(!myrpt->p.authlevel) || myrpt->loginlevel[0]");
				if(debug > 6)
				...
				{
					DEBUG_QUICK("(myrpt->remtxfreqok = check_tx_freq(myrpt))");
					time(&myrpt->last_activity_time);
					telem = myrpt->tele.next;
					while(telem != &myrpt->tele)
					{
						DEBUG_QUICK("telem != &myrpt->tele");
						if(telem->mode == ACT_TIMEOUT_WARNING && !telem->killed)
						{
							DEBUG_QUICK("telem->mode == ACT_TIMEOUT_WARNING && !telem->killed");
							if (telem->chan) {
								DEBUG_QUICK("telem->chan is true, performing soft hangup");
					...
					DEBUG_QUICK("updating RPT_TXKEYED to true");
					rpt_update_boolean(myrpt,"RPT_TXKEYED",1);
		...
		{
			DEBUG_QUICK(" Remote base radio TX unkey");
/* KG7QIN - break 2 */
			myrpt->remotetx = 0;
			...
			DEBUG_QUICK("RPT_TXKEYED");
			rpt_update_boolean(myrpt,"RPT_TXKEYED",0);
		}
		if (myrpt->hfscanmode){
			DEBUG_QUICK("myrpt->hfscanmode");
		...
		if (c && (!myrpt->macrotimer))
		{	
			DEBUG_QUICK("c && (!myrpt->macrotimer)");
			myrpt->macrotimer = MACROTIME;
			memmove(myrpt->macrobuf,myrpt->macrobuf + 1,MAXMACRO - 1);
			if ((c == 'p') || (c == 'P')) {
				DEBUG_QUICK("-- P/p");
		...
		{
			DEBUG_QUICK("if it was a read from incoming");
			f = ast_read(chan);
			if (!f)
			{
				DEBUG_QUICK("LINK: HUNG UP");
			...
			if (f->frametype == AST_FRAME_VOICE)
			{
				...
				if (phone_mode && phone_vox)
				{
					...
					if (n1 != myrpt->wasvox)
					{
						DEBUG_LOGGER(LOG_NOTICE, "Remote vox %d\n", n1);
			...
			if (f->frametype == AST_FRAME_DTMF)
			{
				...
				{
					DEBUG_QUICK("rpt:Hung up");
					if (debug) ast_log(LOG_NOTICE, "@@@@ rpt:Hung Up\n");
					ast_frfree(f);
					break;
				}
			}
			...
			if (f->frametype == AST_FRAME_CONTROL)
			{
				...
				{
					DEBUG_QUICK("f->subclass.integer == AST_CONTROL_RADIO_KEY && (myrpt->newkey <2)");
					if (debug == 7) ast_log(LOG_DEBUG, "@@@@ rx key\n");
					keyed = 1;
					myrpt->rerxtimer = 0;
				}
				...
				if (f->subclass.integer == AST_CONTROL_RADIO_UNKEY)
				{
					DEBUG_QUICK("f->subclass.integer == AST_CONTROL_RADIO_UNKEY");
					myrpt->rerxtimer = 0;
					if (debug == 7) ast_log(LOG_NOTICE, "@@@@ rx un-key\n");
					keyed = 0;
				}
			}
			ast_frfree(f);
		...
		{
			DEBUG_QUICK(" it is was read from radio");
			f = ast_read(myrpt->rxchannel);
			if (!f)
			{
			...
			else if (f->frametype == AST_FRAME_CONTROL)
			{
				...
				{
					DEBUG_QUICK("f->subclass.integer == AST_CONTROL_RADIO_KEY");
					if (debug == 7) ast_log(LOG_DEBUG, "@@@@ remote rx key\n");
					if (!myrpt->remotetx)
					{
						DEBUG_QUICK("remkeyed = 1");
						remkeyed = 1;
					}
				}
				/* if RX un-key */
				if (f->subclass.integer == AST_CONTROL_RADIO_UNKEY)
				{
					if (debug == 7) ast_log(LOG_DEBUG, "@@@@ remote rx un-key\n");
					if (!myrpt->remotetx) 
					{
						DEBUG_QUICK("remkeyed = 0");
						remkeyed = 0;
					}
				}
			}
		...
		{
			DEBUG_QUICK("if is remote mix output");
			f = ast_read(myrpt->pchannel);
			if (!f)
			{
				if (debug) ast_log(LOG_DEBUG, "@@@@ link:Hung Up\n");
		...
		{
			DEBUG_QUICK("do this cuz you have to");
			f = ast_read(myrpt->txchannel);
			if (!f)
			{
				if (debug) ast_log(LOG_DEBUG, "@@@@ link:Hung Up\n");
	...
	if (myrpt->p.archivedir || myrpt->p.discpgm)
	{
		...
		DEBUG_VAR_DATA(mycmd);
		DEBUG_QUICK("-- rpt_update_links(myrpt)");
		rpt_update_links(myrpt);
	...
	DEBUG_VAR_DATA(tmp);
	DEBUG_QUICK("ast_cli_command(nullfd, tmp);");
	...
	if ((myrpt->p.nldisc) && ((strncasecmp(myrpt->rxchannel->name,"radio/", 6) == 0) ||
	    (strncasecmp(myrpt->rxchannel->name,"beagle/", 7) == 0) ||
		    (strncasecmp(myrpt->rxchannel->name,"simpleusb/", 10) == 0)))
	{
		DEBUG_QUICK(" radio/, beagle, or simpleusb/");
		/* go thru all the specs */
	...
	DEBUG_QUICK("**) ast_hangup(myrpt->rxchannel)");
	ast_hangup(myrpt->rxchannel);
	DEBUG_QUICK("closerem(myrpt");
	closerem(myrpt);
	if (myrpt->p.rptnode)
	{
		DEBUG_QUICK("myrpt->p.rptnode");
		rpt_mutex_lock(&myrpt->lock);
		for(i = 0; i < nrpts; i++)
		{
			if (!strcasecmp(rpt_vars[i].name,myrpt->p.rptnode))
...
static int rpt_manager_do_stats(struct mansession *s, const struct message *m, char *str)
{
	...
	for(i = 0; i < nrpts; i++)
	{
		if ((node)&&(!strcmp(node,rpt_vars[i].name))){
			...
			(lastdtmfcommand && strlen(lastdtmfcommand)) ? lastdtmfcommand : not_applicable);
			hours = dailytxtime/3600000L;
			dailytxtime %= 3600000L;
			minutes = dailytxtime/60000L;
			dailytxtime %= 60000L;
			seconds = dailytxtime/1000L;
			dailytxtime %= 1000L;

			astman_append(s, "TxTimeToday: %02d:%02d:%02d:%02d\r\n",
				hours, minutes, seconds, dailytxtime);

			hours = (int) totaltxtime/3600000L;
			totaltxtime %= 3600000L;
			minutes = (int) totaltxtime/60000L;
			totaltxtime %= 60000L;
			seconds = (int)  totaltxtime/1000L;
			totaltxtime %= 1000L;
...
/*! \brief RPT load module - initialization */
static int load_module(void)
{
	int res;
	
	ast_verbose("RPT loading...\n");
